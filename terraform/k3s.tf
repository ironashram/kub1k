module "provision_k3s" {
  ...
  k3s_server_join_args       = local.k3s_server_join_args
  k3s_version                = var.k3s_version
  k3s_install_script_url     = var.k3s_install_script_url
  kube_config_output         = local.kube_config_output
  ssh_user                   = data.vault_generic_secret.ssh.data["user"]
  cluster_name               = var.cluster_name
  control_plane_vip          = var.control_plane_vip
  apiserver_auth_config_yaml = local.k3s_apiserver_auth_config_yaml
}
